class ParameterOptimizer {
  async optimizeParameters(concepts: any[], modules: any[]): Promise<Record<string, any>> {
  ...
  private optimizeForConcept(concept: any, parameters: Record<string, any>) {
  ...
  private optimizeForModule(module: any, parameters: Record<string, any>) {
  ...
  private applyCrossOptimizations(parameters: Record<string, any>) {
  ...
  private calculateComplexity(parameters: Record<string, any>): number {
    let complexity = 1;
    ...
    // Feature complexity
    if (parameters.physics) complexity += 2;
    if (parameters.lighting) complexity += 2;
    if (parameters.morphing) complexity += 1;
    if (parameters.collision) complexity += 1;
    if (parameters.shadows) complexity += 1;
