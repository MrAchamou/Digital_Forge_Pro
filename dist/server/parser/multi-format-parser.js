...
class MultiFormatParser {
    supportedFormats = ['.txt', '.md', '.json', '.csv', '.docx'];
    ...
    async parseContent(content, format, filename) {
        const errors = [];
        let descriptions = [];
        try {
            switch (format) {
    ...
    extractStringsFromObject(obj, descriptions) {
    ...
    parseCsvFile(content) {
        ...
        const descriptionColumnIndex = headers?.findIndex(header => header.includes('description') ||
    ...
    async createEffectJob(description) {
        try {
