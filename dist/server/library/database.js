...
class LibraryDatabase {
    ...
    async search(options = {}) {
        try {
            const { query, category, type, platform, tags, complexity, performance, sortBy = 'createdAt', sortOrder = 'desc', limit = 20, offset = 0 } = options;
            ...
            if (tags && tags.length > 0) {
                filteredEffects = filteredEffects.filter(effect => tags.some(tag => effect.tags.includes(tag)));
            }
            if (complexity !== undefined) {
                filteredEffects = filteredEffects.filter(effect => effect.complexity === complexity);
            }
            if (performance) {
                filteredEffects = filteredEffects.filter(effect => effect.performance === performance);
            }
            // Apply sorting
    ...
    async getSimilarEffects(effectId, limit = 5) {
        try {
    ...
    calculateSimilarityScore(effect1, effect2) {
        let score = 0;
    ...
    async getTrendingEffects(limit = 10) {
        try {
            const allEffectsResult = await storage.getEffects({ limit: 1000 });
            const effects = allEffectsResult.effects;
    ...
    async exportLibrary(options = { format: 'json' }) {
        try {
    ...
    async getRecommendations(recentDownloads, limit = 10) {
        try {
            ...
            const downloadedEffects = await Promise.all(recentDownloads.map(id => storage.getEffect(id)));
            const validDownloaded = downloadedEffects.filter(Boolean);
