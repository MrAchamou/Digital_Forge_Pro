...
import { Sparkles, Database, Brain, Zap, TrendingUp, Cpu, Activity, ChevronRight, Gauge, Network, Shield, Rocket, Atom, Binary } from "lucide-react";
// Mock data for resource usage and module counts if they are not fetched
var averageResourceUsage = { overall: 75.5 }; // Example data
var onlineModules = 5; // Example data
var totalModules = 6; // Example data
export default function Dashboard() {
    ...
    var completionRate = stats ? (stats.effectsGenerated / stats.totalDescriptions) * 100 : 0;
    var getHealthColor = function (value) {
        if (value >= 90)
    ...
    var getHealthBgColor = function (value) {
        if (value >= 90)
            return 'bg-green-500';
        if (value >= 70)
    ...
    return (<div className="space-y-8">
      ...
      <div className="text-center relative">
        ...
        <div className="flex justify-center items-center mt-6 space-x-4">
          <Badge className={"".concat(getHealthBgColor((health === null || health === void 0 ? void 0 : health.overall) || 0), " text-white px-4 py-2")}>
            System Health: {(health === null || health === void 0 ? void 0 : health.overall) !== undefined ? health.overall.toFixed(1) : 'N/A'}%
          </Badge>
          <Badge className="bg-forge-electric text-white px-4 py-2">
            {(stats === null || stats === void 0 ? void 0 : stats.expansionRate) > 0 ? 'AI Expansion Active' : 'Quantum Standby'}
          </Badge>
          <Badge className="bg-forge-plasma text-white px-4 py-2">
            Quality Score: {((stats === null || stats === void 0 ? void 0 : stats.qualityScore) || 0).toFixed(1)}%
          </Badge>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card className="glass-morphism border-forge-cyan/30 bg-transparent relative overflow-hidden">
          ...
          <CardContent className="space-y-4">
            <div className="text-3xl font-bold text-white">
              {((stats === null || stats === void 0 ? void 0 : stats.totalDescriptions) || 0).toLocaleString()}
            </div>
            <div className="text-sm text-gray-300">Total Descriptions</div>
            <Progress value={completionRate} className="h-3"/>
            <div className="flex justify-between text-xs text-gray-400">
              <span>Processed: {(stats === null || stats === void 0 ? void 0 : stats.effectsGenerated) || 0}</span>
        ...
        <Card className="glass-morphism border-forge-purple/30 bg-transparent relative overflow-hidden">
          ...
          <CardContent className="space-y-4">
            <div className="text-3xl font-bold text-white">
              {(stats === null || stats === void 0 ? void 0 : stats.averageGenerationTime) !== undefined ? stats.averageGenerationTime.toFixed(1) : 'N/A'}ms
            </div>
            ...
            <Badge className="bg-green-500 text-white">
              Success: {((stats === null || stats === void 0 ? void 0 : stats.successRate) || 0).toFixed(1)}%
            </Badge>
          </CardContent>
        </Card>
        ...
        <Card className="glass-morphism border-forge-plasma/30 bg-transparent relative overflow-hidden">
          ...
          <CardContent className="space-y-4">
            <div className="text-3xl font-bold text-white">
              {(stats === null || stats === void 0 ? void 0 : stats.expansionRate) !== undefined ? stats.expansionRate.toFixed(1) : 'N/A'}
            </div>
            ...
            <Badge className={"".concat((stats === null || stats === void 0 ? void 0 : stats.expansionRate) > 0 ? 'bg-forge-plasma' : 'bg-gray-500', " text-white")}>
              {(stats === null || stats === void 0 ? void 0 : stats.expansionRate) > 0 ? 'ACTIVE' : 'STANDBY'}
            </Badge>
          </CardContent>
        </Card>
        ...
        <Card className="glass-morphism border-forge-gold/30 bg-transparent relative overflow-hidden">
          ...
          <CardContent className="space-y-4">
            <div className="text-3xl font-bold text-white">
              {(health === null || health === void 0 ? void 0 : health.overall) !== undefined ? health.overall.toFixed(1) : 'N/A'}%
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <Card className="glass-morphism border-forge-purple/30 bg-transparent">
          ...
          <CardContent className="space-y-3">
            ...
            return (<div key={name} className="flex items-center justify-between p-3 bg-forge-dark/30 rounded-lg">
                <div className="flex items-center gap-3">
                  <div className={"w-3 h-3 rounded-full ".concat(getHealthBgColor(module.performance))}/>
                  <span className="text-white font-medium capitalize">{name}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className={"text-sm ".concat(getHealthColor(module.performance))}>
                    {module.performance.toFixed(1)}%
                  </span>
                  <Badge className={"".concat(getHealthBgColor(module.performance), " text-white text-xs")}>
                    {module.status}
                  </Badge>
                </div>
              </div>);
